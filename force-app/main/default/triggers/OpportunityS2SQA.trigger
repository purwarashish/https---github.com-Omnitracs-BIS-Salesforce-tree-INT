trigger OpportunityS2SQA on Opportunity (before insert,before update) {
    ...
    if(trigger.isbefore && trigger.size==1){
        if(trigger.new[0].ConnectionReceivedId != null && trigger.new[0].ConnectionReceivedId == '04P3F000000007hUAA'){
            if(trigger.new[0].RecordType_S2S__c != null && trigger.new[0].RecordType_S2S__c != ''){
                trigger.new[0].RecordTypeId = Schema.SObjectType.Opportunity.getRecordTypeInfosByDeveloperName().get(trigger.new[0].RecordType_S2S__c).getRecordTypeId();
            }
            ...
            try{
                ...
                if(trigger.new[0].Company_Transferred_from_Legacy_S2S__c != null && trigger.new[0].Company_Transferred_from_Legacy_S2S__c != ''){
                   ...
                   prnc1 = [SELECT LocalRecordId FROM PartnerNetworkRecordConnection WHERE ConnectionId='04P3F000000007hUAA' AND Status='Received' AND PartnerRecordId=:trigger.new[0].Company_Transferred_from_Account_S2S__c LIMIT 1];
                   if(prnc1.size()>0)
                ...
                if(trigger.new[0].Eval_Parent_Opportunity_Legacy_S2S__c != null && trigger.new[0].Eval_Parent_Opportunity_Legacy_S2S__c != ''){
                   ...
                   prnc2 = [SELECT LocalRecordId FROM PartnerNetworkRecordConnection WHERE ConnectionId='04P3F000000007hUAA' AND Status='Received' AND PartnerRecordId=:trigger.new[0].Eval_Parent_Opportunity_S2S__c LIMIT 1];
                   if(prnc2.size()>0)
                ...
                if(trigger.new[0].Primary_Contact_Legacy_Id_S2S__c != null && trigger.new[0].Primary_Contact_Legacy_Id_S2S__c != ''){
                   ...
                   prnc3 = [SELECT LocalRecordId FROM PartnerNetworkRecordConnection WHERE ConnectionId='04P3F000000007hUAA' AND Status='Received' AND PartnerRecordId=:trigger.new[0].Primary_Contact_Id_S2S__c LIMIT 1];
                   if(prnc3.size()>0)
