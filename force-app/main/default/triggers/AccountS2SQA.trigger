trigger AccountS2SQA on Account (before insert, before update) {
    ...
    if(trigger.size==1){
        if(trigger.new[0].ConnectionReceivedId != null && trigger.new[0].ConnectionReceivedId == '04P3F000000007hUAA'){
            if(trigger.new[0].Recordtype_S2S__c=='Partner'){
                trigger.new[0].RecordTypeId = Schema.SObjectType.Account.getRecordTypeInfosByName().get('Reseller').getRecordTypeId();
                    }else if(trigger.new[0].Recordtype_S2S__c=='Sales'){
                        trigger.new[0].RecordTypeId = Schema.SObjectType.Account.getRecordTypeInfosByName().get('T&L Sales Accounts').getRecordTypeId();
                    }
            ...
            for(Account a:trigger.new){
                ...
                if(a.ParentId == null && parentNetSuiteMap.containsKey(a.Parent_Id_S2S__c)){
                ...
                if(a.Support_Account__c == null && supportNetSuiteMap.containsKey(a.Support_Account_Id_S2S__c)){
