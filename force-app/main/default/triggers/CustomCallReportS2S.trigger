trigger CustomCallReportS2S on Custom_Call_Report__c (after insert,after update) {
    ...
    if(trigger.isafter && trigger.size==1){
        if(trigger.new[0].ConnectionReceivedId != null && trigger.new[0].ConnectionReceivedId == '04P3F000000007cUAA'){
            ...
            if(trigger.new[0].Account_Legacy_S2S__c != null && trigger.new[0].Account_Legacy_S2S__c != ''){
                ...
                prnc = [SELECT LocalRecordId FROM PartnerNetworkRecordConnection WHERE ConnectionId='04P3F000000007cUAA' AND Status='Received' AND PartnerRecordId=:trigger.new[0].Account_S2S__c];
                    if(prnc.size()>0)
            ...
            if(trigger.new[0].Contact_Legacy_Id_S2S__c != null && trigger.new[0].Contact_Legacy_Id_S2S__c != ''){
                ...
                prnc2 = [SELECT LocalRecordId FROM PartnerNetworkRecordConnection WHERE ConnectionId='04P3F000000007cUAA' AND Status='Received' AND PartnerRecordId=:trigger.new[0].Contact_S2S__c];
                if(prnc2.size()>0)
